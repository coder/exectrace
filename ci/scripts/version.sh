...
# This script generates the version string used by exectrace v2, including for
# dev versions. Note: the version returned by this script will NOT include the
# "v" prefix that is included in the Git tag.
#
...
if [[ "${CODER_RELEASE:-}" == *t* ]]; then
	...
	if [[ "$last_tag" != "$(git describe --always)" ]]; then
		...
		if [[ "$(ps -o comm= "$PPID" || true)" == *make* ]]; then
			echo "ERROR: version.sh: the current commit is not tagged with an annotated tag"
			kill "$PPID" || true
			exit 1
		fi

		echo "version.sh: the current commit is not tagged with an annotated tag"
		exit 1
	fi
else

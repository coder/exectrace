...
if ! command -v gh; then
	error "The GitHub CLI is required to run this script"
fi

# realpath-ify all input files so we can cd to the repo root below.
files=()
for f in "$@"; do
	if [[ ! -e "$f" ]]; then
...
# cd to the repo root so we can run git commands and other scripts.
cd "$(dirname "$0")/.."
...
pushd "$temp_dir"
sha256sum ./* | sed -e 's/\.\///' - >"exectrace_${version}_checksums.txt"
popd
